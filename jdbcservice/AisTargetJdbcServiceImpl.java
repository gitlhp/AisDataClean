...
public class AisTargetJdbcServiceImpl implements AisTargetJdbcService {

    private AisTargetDao aisTargetDao;

    public AisTargetJdbcServiceImpl(AisTargetDao aisTargetDao){
        this.aisTargetDao = aisTargetDao;
    }

    @Override
    public List<Integer> getShipIdByDongHisA() throws Exception {
        return aisTargetDao.getShipIdByDongHisA();
    }

    @Override
    public int getMmsiByDongLastab(int shipid) throws Exception {
        return aisTargetDao.getMmsiByDongLastab(shipid);
    }
            ...
            for (int i= 0;i<dongHisAList.size();i++){
                ...
                for (int j = 0;j<staticHisAList.size();j++){//
                    ...
                    if (dongHisAList.get(i).getRecord_gps_Time()<staticHisAList.get(0).getStart_Datetime()){
                        ...
                        //再将合并后的适配表和另一张静表合并iden表
                        getShipTarget_OrighisVoListLast(shipTarget_orighisVo,staticHisAIdenList);
                        //将填入信息的适配表添加进目标适配表集合中
                        ShipTarget_OrighisVoList.add(shipTarget_orighisVo);
                        //释放内存
                        shipTarget_orighisVo = null;
                        //跳出本轮循环
                        break;
                    }else if (dongHisAList.get(i).getRecord_gps_Time()>=staticHisAList.get(0).getStart_Datetime() && j+1 < staticHisAList.size()){//当动点处于静点时间之间，且保证不会越界index.....
    ...
    public int insertShipTarget_Orighis() throws Exception{
        ...
        return aisTargetDao.insertShipTarget_Orighis(list1);
    }
    ...
    public List<ShipTarget_Orighis> getShipTarget_OrighisList() throws Exception{
        return aisTargetDao.getShipTarget_OrighisList();
    }
